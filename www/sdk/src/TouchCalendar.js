 ...
 */
/**
 * @class Ext.TouchCalendar
 * @author Stuart Ashworth
 *
 * For use with Sencha Touch 2
 * 
 * This extension wraps the Ext.TouchCalendarView in a Ext.Carousel component and allows the calendar to respond to swipe
 * gestures to switch the displayed period. It works by creating 3 Ext.TouchCalendarViews and dynamically creating/removing
 * views as the user moves back/forward through time. 
 * 
 * ![Ext.TouchCalendar Screenshot](http://www.swarmonline.com/Ext.TouchCalendar/screenshots/Ext.TouchCalendar-month-ss.png)
 * 
 * [Ext.TouchCalendar Demo](http://www.swarmonline.com/wp-content/uploads/Ext.TouchCalendar/examples/Ext.TouchCalendar.html)
 * 
 */

Ext.define('Ext.TouchCalendar',{
	...
	*/
	...
	/**
	* @cfg {Boolean} enableSimpleEvents True to enable the Ext.TouchCalendarSimpleEvents plugin. When true the Ext.TouchCalendarSimpleEvents JS and CSS files
	...
	*/
	...
	/**
	* @cfg {Boolean} enableEventBars True to enable the Ext.TouchCalendarEvents plugin. When true the Ext.TouchCalendarEvents JS and CSS files
	* must be included and an eventStore option, containing an Ext.data.Store instance, be given to the viewConfig.
	*/
	...
	*/
	getViewConfig: function(viewValue){
		...
		if(this.enableSimpleEvents){
			plugins.push(new Ext.TouchCalendarSimpleEvents());
		} else if (this.enableEventBars){
			plugins.push(new Ext.TouchCalendarEvents());
		}
	...
	initViews: function(){
		...
		items.push(
		    new Ext.TouchCalendarView(Ext.applyIf({
		        currentDate: viewValue
		...
		// active view
		items.push(
		    new Ext.TouchCalendarView(this.getViewConfig(origCurrentDate))
		);
	...
	*/
	getValue: function(){
	    /* NOTE: JACOB CHANGED THIS FUNCTION BY ADDING THIS FIRST LINE */
	...
	onActiveItemChange: function(container, newCard, oldCard){
		if (this.enableSwipeNavigate) {
			...
			if (direction === 'forward') {
				...
				var newCalendar = new Ext.TouchCalendarView(this.getViewConfig(Ext.Date.add(newCard.currentDate, Ext.Date[this.viewMode], 1)));
